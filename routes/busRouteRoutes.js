...
router.post('/', async (req, res) => {
  try {
    ...
    if (waypoint_groups && waypoint_groups.length > 0) {
      // Expand waypoint groups into stops
      ...
      for (const groupRef of sortedGroups) {
        ...
        // Add waypoints from this group as stops
        for (const waypoint of group.waypoints) {
          ...
          // Estimate distance (you can improve this with actual calculations)
          cumulativeDistance += 10; // Rough estimate: 10km between stops
        }
      }
...
router.put('/:route_id', async (req, res) => {
  try {
    ...
    // Don't allow changing route_id
    delete updates.route_id;
    
    // If waypoint groups are updated, expand them
    if (updates.waypoint_groups && updates.waypoint_groups.length > 0) {
      ...
      // If manual stops are updated, recalculate total distance
